...
console.log('🔍 Checking Rejected Records');
console.log('============================');
console.log(`Total lines: ${lines.length}`);
console.log(`Parsed records: ${parsedRecords.length}`);
console.log(`Rejected records: ${lines.length - parsedRecords.length}`);
...
lines.forEach((line, index) => {
  if (line.length >= 11) {
    ...
    const isFieldIdAlphanumeric = /^[A-Za-z0-9]{4}$/.test(fieldId);
    
    if (!isLengthNumeric || !isRecordTypeNumeric || !isFieldIdAlphanumeric) {
      console.log(`Line ${index + 1}: "${line}"`);
      console.log(`  Length: ${length} (numeric: ${isLengthNumeric})`);
      console.log(`  Record Type: ${recordType} (numeric: ${isRecordTypeNumeric})`);
      console.log(`  Field ID: ${fieldId} (alphanumeric: ${isFieldIdAlphanumeric})`);
      console.log('');
    }
  } else {
    console.log(`Line ${index + 1}: "${line}" (too short)`);
    console.log('');
  }
});